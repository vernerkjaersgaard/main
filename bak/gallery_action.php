    ...
    foreach ($chunks as $chunk)
    {
        ...
        foreach ($chunk as $filename)
        {
            ...
            else
            {
                ...
                $tmp_resized = tempnam(sys_get_temp_dir(), 'resize_');
                $resized->save($tmp_resized);

                $zip->addFile($tmp_resized, $filename);
                // Note: can't unlink $tmp_resized yet — ZipArchive reads it
                // lazily and needs it to still exist until $zip->close() below.
            }
        }

        $zip->close();

        // Now safe to clean up any temp resized files from this chunk
        if ($action !== 'download_full')
        {
            foreach ($chunk as $filename)
            {
                // (handled per-file below instead — see note)
            }
        }
